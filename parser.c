...
struct parse_result parse_message(unsigned char buffer[BUFFER_SIZE]) {
    ...
    if(abs(current_time - timestamp) <= 5) {
        logger("parse_message: the timestamp is within allowed time difference of 5 seconds.");
    } else {
        logger("parse_message: Error! Timestamp difference is too big (%d seconds.)! Aborting.", abs(current_time - timestamp));
